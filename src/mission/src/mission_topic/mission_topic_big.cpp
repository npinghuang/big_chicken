...
float doing_time_31;
float wait_sec = 23.0; //wait for little chicken 10.0
float wait_sec_2 = 3.0; //wait for little chicken 10.0
...
void chatterCallback(const mission::maintomission::ConstPtr& msg)
{
    ...
    if (msg->emerg == 1){
        ...
        switch (msg->action)
        {
            ...
            case 32:{ // delay wait for big chicken to walk away
                if (state_planer == 1){
                    if (mission_wait.count == 0){
                        begin_time = ros::WallTime::now();
                        mission_wait.count = 1;
