...
pose_tracker::pose_tracker(){
  ...
  n.param<float>("v_dacc_tolerance", v_dacc_tolerance, 0.05);
...
void pose_tracker::rotate(float g_theta, float n_theta){
  ...
  ROS_INFO_STREAM("g_theta : "<<g_theta);
  ROS_INFO_STREAM("ｎ_theta : "<<n_theta);
  ROS_INFO_STREAM("result : "<<rotate_state);
...
void pose_tracker::set_line_func(){ //note theta is atan(-a,b)
  ...
  Marker_publish(now_pose_.x,now_pose_.y,goal_pose_.x,goal_pose_.y);
...
float pose_tracker::compensate_angular_vel(){//要重新整理
  if( linear_compensate ){
    ...
    if(fabs(w_now) >= fabs(w_max)){
      ROS_INFO("w too fast, reduce to w_max");
      w_now = w_max*w_now/fabs(w_now);
    }
...
void pose_tracker::Marker_publish(float x1, float y1, float x2, float y2){
